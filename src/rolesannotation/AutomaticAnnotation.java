...
public class AutomaticAnnotation {
    ...
    private ArrayList<String> setAnnotationWithWikiMedia(List<String> names){
        ...
        for(String name : names){
            try{
                ...
                JsonReader jReader = Json.createReader(ProxyConnection.getInstance().getConnection(url, useProxy).getInputStream());
    ...
    private ArrayList<String> setAnnotationWithWikiData() throws MalformedURLException, IOException{
        ...
        for(String name : titlesMap.keySet()){
            ...
            if(titlesMap.get(name).startsWith("Q")){                                //make sure that the title is a valid WikiData ID
                URL url = new URL(wikiDataEntity + titlesMap.get(name));
                JsonObject jsonData = Json.createReader(ProxyConnection.getInstance().getConnection(url, useProxy).getInputStream()).readObject().getJsonObject("entities").getJsonObject(titlesMap.get(name));
    ...
    public void setWikiIds(List<String> names) throws MalformedURLException, IOException{
        ...
        for (String name : names){
            ...
            ProxyConnection urlConn = ProxyConnection.getInstance();
            JsonObject jsonData = Json.createReader(urlConn.getConnection(tempUrl, useProxy).getInputStream()).readObject().getJsonObject("query").getJsonObject("pages");   //jsonobject key batchcompleted / query
        ...
        if(idsFile == null){
            //if linux
            idsFile = new File(outputFile.getParent() + "/" + "wikiIds.csv");  
            //if windows
            //idsFile = new File(outputFile.getParent() + "\\" + "wikiIds.csv");  
        }
        
        ArrayList<String> idsToSave = new ArrayList<>();
        idsToSave.add("Name,WikiTitle,WikiPageId");
        for(String name : titlesMap.keySet()){
            idsToSave.add(wikiNameMap.get(name) +"," + titlesMap.get(name) + "," + pageIdsMap.get(name));
        }
        Utils.writeListToFile(idsToSave, idsFile);
    ...
    public void setIdsFile(File idsFile){
        if(idsFile == null){
            throw new IllegalArgumentException("File for ids is null");
        }
    ...
    public void execute() throws Exception{
        ...
        System.out.println("People to annotate: " + namesOnFile.size());
        long startTime = System.currentTimeMillis();
        setWikiIds(namesOnFile);
        long stopTime = System.currentTimeMillis();
        System.out.println("Getting id: " + (stopTime - startTime));
        ...
        if(wiki2Use == DataBaseWiki.WIKIDATA){
            ...
            System.out.println("WikiData: " + (stopTime - startTime));
        }else if(wiki2Use == DataBaseWiki.MEDIAWIKI){
            startTime = System.currentTimeMillis();
            missingPersons = setAnnotationWithWikiMedia();
            stopTime = System.currentTimeMillis();
            System.out.println("WikiMedia: " + (stopTime - startTime));
        }else{
            startTime = System.currentTimeMillis();
            missingPersons = setAnnotationWithWikiData();
            ...
            stopTime = System.currentTimeMillis();
            System.out.println("Both wikis: " + (stopTime - startTime));
        }
        writeResultToFile();
